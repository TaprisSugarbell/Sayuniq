...
async def get_tioanime_servers(chapter_url):
    async with aiohttp.ClientSession(headers=USER_AGENT) as session:
        async with session.get(chapter_url) as r:
            logging.info(f'Get {chapter_url} is "{r.ok}"')
            soup = BeautifulSoup(await r.content.read(), PARSER)
            _script = "".join(
                _ist.string
                for _ist in soup.find_all("script")[10:]
                if getattr(_ist, "string")
            )
            _anime_uri = (
                soup.find(
                    "div",
                    attrs={"class": "episodes-nav d-flex justify-content-center mb-4"},
                )
                .find_all("a")[1]
                .get("href")
            )
    ...
    async with aiohttp.ClientSession(headers=USER_AGENT) as session:
        async with session.get(chapter_url) as r:
            ...
            _lks = [
                ie.string
                for ie in _script
                if getattr(ie, "string") and "var video = [];" in ie.string
            ][0]
            ...
            for _link in _links:
                ...
                match mode:
                    case "um2.php":
                        _r = await session.get(_link, headers={"referer": chapter_url})
                        _soup = BeautifulSoup(await _r.content.read(), PARSER)
                        ...
                        _r2 = await session.post(
                            "https://jkanime.net/gsplay/api.php", data={"v": _v}
                        )
                        _servers.append([await _r2.json()][0]["file"])
                    case "um.php":
                        _r = await session.get(_link)
                        _soup = BeautifulSoup(await _r.content.read(), PARSER)
                        _servers.append(
                            _lnks.findall(_soup.find_all("script")[-1].string)[0]
                        )
                    case "jk.php":
                        _r = await session.get(_link)
                        _soup = BeautifulSoup(await _r.content.read(), PARSER)
                    ...
                    case "jkokru.php":
                        _servers.append(
                            "https://ok.ru/videoembed/" + _link.split("u=")[-1]
                        )
                    case "jkfembed.php":
...
async def get_mc_servers(chapter_url):
    async with aiohttp.ClientSession(headers=USER_AGENT) as session:
        async with session.get(chapter_url) as r:
            ...
            _bb = [
                oei.get("data-player")
                for oei in soup.find("div", attrs={"class": "playother"}).find_all("p")
            ]

            b64_decoded = [
                oy.split("?url=")[-1]
                for oy in [base64.b64decode(oe).decode() for oe in _bb]
            ]
            b64_decoded.extend(
                [
                    ei.get("href")
                    for ei in soup.find("div", attrs={"class": "downbtns"}).find_all(
                        "a"
                    )
                ]
            )
...
async def get_flv_servers(chapter_url):
    ...
    return [
        ou.get("code")
        for ou in json.loads(
            re.findall(r"\{\"SUB.*}", soup.find_all("script")[-7].string)[0]
        )["SUB"]
    ]
