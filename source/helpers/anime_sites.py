...
from .site_assistant import SitesAssistant
...
from ..config import BOT_NAME, USER_AGENT
import string
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(_url_base) as result:
            ...
            for _a in list_of_a[::-1]:
                chapter_no = [
                    i for i in re.findall(r"[\d.]*", _a.find("h3").text) if i
                ][-1]
                title = _a.find("h3").text.replace(chapter_no, "").strip()
                chapter_url = _url_base[:-1] + _a.get("href")
                _sa = SitesAssistant(
                    _site, title, None, chapter_no, _database=db, app=app
                )
                ...
                if _c:
                    ...
                    anime_url = _url_base[:-1] + _anime_uri
                    try:
                        await database_assistant(_sa, servers, anime_url, chapter_url)
                    except Exception as e:
                        await bot_error(e, app)
...
async def monoschinos(app):
    ...
    async with aiohttp.ClientSession(headers=USER_AGENT) as session:
        async with session.get(_url_base) as r:
            ...
            list_of_a = soup.find("div", attrs={"class": "row row-cols-5"}).find_all(
                "a"
            )
            for _a in list_of_a[::-1]:
